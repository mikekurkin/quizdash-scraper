...
export class GitHubStorage extends CsvStorage {
  ...
  constructor() {
    ...
    if (!token || !this.owner || !this.repo) {
      throw new StorageError(
        "GitHub configuration is incomplete. Please check environment variables."
      );
    }

    this.octokit = new Octokit({ auth: token });
    logger.info(
      `Initialized GitHub storage for repository ${this.owner}/${this.repo} (${this.branch})`
    );
  }
  ...
  async initialize(): Promise<void> {
    ...
    const nonMetaFiles = dirFiles.filter(
      (f) => f !== ".git" && f !== ".gitattributes" && !f.startsWith(".DS_")
    );
    if (nonMetaFiles.length === 0) {
      logger.info(
        "Data directory is empty, attempting to clone from GitHub..."
      );
      try {
        ...
        logger.warn(
          "Could not clone from GitHub, proceeding with initialization.",
          err
        );
      }
    }
    ...
    if (!isRepo) {
      ...
      await this.runGitCommand(
        "commit",
        "-m",
        '"Initial commit with existing data"'
      );
  ...
  async pullFromGitHub(): Promise<void> {
    ...
    try {
      ...
      throw new StorageError(
        "Failed to pull latest changes from GitHub",
        error
      );
    }
  }
  ...
  private async handleMergeConflicts(): Promise<void> {
    ...
    await this.runGitCommand(
      "commit",
      "-m",
      '"Resolve merge conflicts keeping local changes"'
    );
  }
  ...
  async saveGames(games: Game[]): Promise<void> {
    ...
    logger.debug(
      `Marked games file for sync: ${path.relative(
        config.storage.path,
        this.gamesFile
      )}`
    );
  ...
  async saveResults(results: GameResult[]): Promise<void> {
    ...
    logger.debug(
      `Marked results file for sync: ${path.relative(
        config.storage.path,
        this.resultsFile
      )}`
    );
  ...
  async saveSeries(series: Series): Promise<void> {
    ...
    logger.debug(
      `Marked series file for sync: ${path.relative(
        config.storage.path,
        this.seriesFile
      )}`
    );
  ...
  async saveTeam(team: Team): Promise<void> {
    ...
    logger.debug(
      `Marked teams file for sync: ${path.relative(
        config.storage.path,
        this.teamsFile
      )}`
    );
  ...
  async updateCityLastGameId(
    ...
    logger.debug(
      `Marked cities file for sync: ${path.relative(
        config.storage.path,
        this.citiesFile
      )}`
    );
